...
var Tab={
    addTab:function(title,url){
       var index = url.replace('.', '_').replace(/\//g, '__');
		console.log(index);
       alert($('#tab_' + index).length);
        if ($('#tab_' + index).length == 0) {
            // 追加li Tab元素
            var tab = '<li id="tab_' + index +'" data-index="' + index + '"  class="cur"><a class="" href="javascript:;">' + title + '<i class="fa fa-times icon tabClose" onClick="Tab.removeTab(this);"></i>  </a></li>';//<i class="zmdi zmdi-close"></i><
            $('.page-header .breadcrumb').append(tab);
           
            // $('.myIframe').removeClass('cur');
            $('.breadcrumb li').removeClass('cur');
            // 切换iframe的路径
    ...
    removeTab:function($this){
        ...
        // 拿到li下的url路径
        var url= $($this).parent().parent().attr('data-index');
        // var index = url.replace('.', '_').replace(/\//g, '__');   
        // 拿到Tab选项卡的长度
        var len=$('.breadcrumb li').length;
        
        var i= $item.parent().parent().attr('data-index');
        //var point=$item.parent().parent().parent();
         // alert($(this).index);

         for(var j=0;j<len.length;j++){     
            alert(j+"jjjjjj");  
            alert("222");
            // var abc= $('breadcrumb li').eq(j).val();
            alert(abc);
         };
         $('.breadcrumb li').each(function(n) {
             alert(n+"nnnnnnn");
         });
         // var i=$('.breadcrumb li').length;
         alert(i+"=======");
        ...
        // alert(index);
            $('#myIframe_'+index).remove(); 
             $item.remove();
                 //通过遍历给菜单项加上data-index属性
        $(".breadcrumb li").each(function (index) {
            if (!$(this).attr('index')) {
                $(this).attr('index', index);
                alert($(this).attr('index'+'dddddddddddddddd'));
            }
        });
...
$(function(){
	...
	* $('#title li').click(function(){
	 $('#title li').removeClass('change');
	 $(this).addClass('change');
	 $('#content li').removeClass('change').hide();
	 $('#content li').eq($(this).index()).addClass('change').show();
	 });
	* */


	var toggleBtn = true;
    $('#tabBox .toggleTab').unbind('click').click(function(event){

		var aa = $(this);
		if(!$(this).hasClass("toggleTabActive")){
			$(this).addClass("toggleTabActive");
			var index = $(this).index();
			$(this).children('.TabChildBox').slideToggle();
			var tab = '<li class="erJiLi"><i class="fa fa-home"></i><a href="javascript:void(0);" class="navLi navLi'+index+'"><i class="fa fa-times icon tabClose"></i></a></li>';
			$('.page-header .breadcrumb').append(tab);
			$('.page-header .breadcrumb .navLi'+index).html($(this).find('.menu-text').html().toUpperCase()+' <i class="fa fa-times icon tabClose"></i>');
		}else{
			$(this).children('.TabChildBox').slideToggle();
		}

		$('.page-header .breadcrumb .erJiLi').click(function(){
			$(this).remove();
			aa.removeClass("toggleTabActive");
		});
	});
...
// 选项卡的关闭
// $('.breadcrumb .tabClose').click(function() {
//     alert("1"); 
// });