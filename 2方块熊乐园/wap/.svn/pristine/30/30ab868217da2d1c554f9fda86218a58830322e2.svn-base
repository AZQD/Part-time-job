	...
	if phone and string.len(phone)>1 and pass and string.len(pass) then
		...
		if res then
			if table.maxn(res) > 0 then
				out_str = out_str..'已经注册'
				ngx.header['content-length'] = #out_str
				ngx.say(out_str)
				return 200
			end
		end
		sql = "insert into info_gg_users(`name`,`phone`,`pass`,`reg`,`last`,`salt`) VALUES('',"..ngx.quote_sql_str(phone)..","..ngx.quote_sql_str(pass)..",now(),0,"..ngx.quote_sql_str(str.to_hex(ran_str.bytes(3)))..")"
		--ngx.say(sql)
		local res, err = mysql.query(sql)
		if res then
			out_str = out_str..'注册成功'
		else
			--验证是否插入成功
