...
class MembersController extends FrontendController{
	public function _initialize() {
		parent::_initialize();
        //访问者控制
        // echo $this->visitor->is_login;die;
        // echo IS_AJAX;die;
    ...
    public function test(){
        // cookie("facebookData","fsdfsdf");
        // PRINT($_COOKIE;
        // setcookie("facebookData","fsdfsdf",time()+86400);
        // echo $_COOKIE['facebookData'];
        // session("ttt",)
        // echo  cookie("facebookData");
        echo $_COOKIE['facebookData'];
    }
    ...
    public function facebookLogin(){ 
         ...
         $first_name = I('post.first_name','','trim');
         $last_name = I('post.last_name','','trim');
         $facebookid = I('post.id','','trim');
         if(!$facebookid) $this->ajaxReturn(0,'facebookid is null');
         $facebookData=["username"=>time(),"first_name"=>$first_name,"last_name"=>$last_name,"facebookid"=>$facebookid,"email"=>$facebookid."@qq.com","password"=>$facebookid,"pwd_hash"=>"?:m5a7","utype"=>2];
         $member=M('Members')->where(array('facebookid'=>$facebookid))->find();
         ...
         if($member){ //登陆成功
            ...
            if(false === $data = $passport->register($facebookData)){
    ...
    public function register(){
        ...
        if(IS_POST && IS_AJAX){
            ...
            $facebookData=json_decode($_COOKIE['facebookData'],true);
            if($facebookData){
                // $facebookData=$facebookData,true);
            }
