...
function _M.main()
	...
	local sql = "SELECT `id`,`uid`,`title`,`goodstatus`,`price`,`imgs` ,`cimg`,`details`,`contactname`,`cid`,`issell`,`addr_l1`,`addr_l2`,`addr_l3`,`addr_l4`,`views`,`wishs`, DATE_FORMAT(`pubtime`,'%m/%d/%Y') as pubtime,`status`  FROM `info_gg_publist` WHERE status=0 "
	...
	--总条数
	local msql = "select count(id) as n from info_gg_publist where status=0 "..where_ext.." and details like "..ngx.quote_sql_str('%'..key..'%')
	...
	local sql = sql..where_ext.." and (details like "..ngx.quote_sql_str('%'..key..'%') .. " or title like "..ngx.quote_sql_str('%'..key..'%')..")"
