    ...
    $.ajax({
        ...
        success: function(data){
            if(data.status == 200){
                ...
                /*
                 *
                 * <div class="col-xs-2 col-sm-2 left">CATEGORY</div>
                 .sortBox .sort .partInner2 .partInner2Box .categoryBox
                 <div class="col-xs-10 col-sm-10 categoryBox right">
                 <select class="location category1">
                 <!--<option value="LUSAKA">LUSAKA</option>-->
                 </select>
                 <select class="location category2">
                ...
                for (var sort in data){
                    ...
                    $('.sortBox .sort .partInner2 .partInner2Box .categoryBox .category1').append(
                        '<option value='+data[sort].name+'>'+data[sort].name+'</option>'
                    );
                }
                ...
                for(var i=0; i<chushiSubArr.length; i++){
                    $('.sortBox .sort .partInner2 .partInner2Box .categoryBox .category2').append(
                        '<option value='+chushiSubArr[i].name+'>'+chushiSubArr[i].name+'</option>-->'
                    );
                }
                cid = chushiSubArr[0].cid;
                console.log('cid='+cid);
                //初始化：没有点击一级菜单时，直接点击二级菜单
                $('#category2').unbind('change').change(function(){
                    var secondIndex = document.getElementById("category2").selectedIndex;
//                        alert(secondIndex);
                ...
                //点击一级菜单
                var firstIndex = 0;
                $('.sortBox .sort .partInner2 .partInner2Box .categoryBox .category1').unbind('change').change(function(){
                    var categoryName = '';
                    var index = document.getElementById("category1").selectedIndex;
                    var nowSubArr = data[index].sub;
                    $('.sortBox .sort .partInner2 .partInner2Box .categoryBox .category2').empty();
                    for(var i=0; i<nowSubArr.length; i++){
                        $('.sortBox .sort .partInner2 .partInner2Box .categoryBox .category2').append(
                            '<option value='+nowSubArr[i].name+'>'+nowSubArr[i].name+'</option>-->'
                        );
                    }
                    ...
                    console.log('cid='+cid);
                    //点击二级菜单//不显示默认地址
                    $('.sortBox .sort .partInner2 .partInner2Box .categoryBox .category2').unbind('change').change(function(){
                        var categoryName2 = '';
                        var secondIndex = document.getElementById("category2").selectedIndex;
...
//            alert(issell);
    ...
    $('.sortBox .sort .partInner2 .partInner2Box .right .priceText').blur(function(){
        ...
        if(priceText>=0 && priceText <=100000000){
            ...
            commonPopFun1("The price should be in the range of 0 to 100000000", 'Failed');
                ...
                if(data.status == 200){
                    ...
                    for(var i=0; i<updateOldImgArr.length; i++){
                        $('.sortBox .sort .partInner2 .partInner2Box .right .imagesBox').prepend(
                            '<div class="nowHaveImg" style="background: url('+baseImgSrc+updateOldImgArr[i]+') no-repeat center center;background-size:100% auto;"><img class="closeNowHaveImg" src="image/publish/close.png"/></div>'
                        );
                    }
...
//                                        window.location.href = "detail.html?id="+updatepubcontentId;
                ...
                console.log(JSON.stringify(pubGoodInfo));
...
//                                alert(data.status);
            ...
            commonPopFun1("Please complete all of the details to publish","Failed");
        }
