...
function pager($count, $pagesize) {
    ...
    $pager->rollPage = 5;
    $pager->setConfig('first', 'FIRST');
    $pager->setConfig('prev', 'PREV');
    $pager->setConfig('next', 'NEXT');
    $pager->setConfig('last', 'LAST');
...
function url_rewrite($alias=NULL,$get=NULL,$subsite_id=0,$rewrite=true){
    ...
    if($page_list[$alias]['url']=='0' || $rewrite==false){//原始链接
        ...
        $sub_domain = $domain_rules[$_SERVER['HTTP_HOST']];
        if($sub_domain){
            strtolower($sub_domain) != strtolower($page_list[$alias]['module']) && $u = $page_list[$alias]['module'].'/';
        }elseif(MODULE_NAME != C('DEFAULT_MODULE') || strtolower($page_list[$alias]['module']) != strtolower(MODULE_NAME)){
            $u = $page_list[$alias]['module'].'/';
        }
